    ...
    <x-authentication-card>
        ...
        <form action="{{ route('blogs.update', $blog) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" class="block mt-1 w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:border-blue-500" name="title" id="title" class="form-control" value="{{ $blog->title }}">
            </div>


            <div class="mb-4">
                <label for="featured_image" class="block font-semibold text-gray-700">Featured Image:</label>
                <input type="file" name="featured_image" id="featured_image" required class="w-full">
            </div>


            <div class="form-group">
                <label for="content" class="block font-semibold text-gray-700">Content:</label>
                <textarea name="content" id="content" class="form-control block mt-1 w-full  px-3 py-2 border border-gray-300 rounded-md resize-none focus:outline-none focus:border-blue-500" required>
                    {!! $blog->content !!}
                </textarea>
            </div>
